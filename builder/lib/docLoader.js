...
const template = (text, values, opts) => {
    ...
    const rule = new RegExp(`${rescape(opts.left)}([\\w\\s\\.\\(\\)\\|-]+)${rescape(opts.right)}`, 'g');
...
class Renderer extends marked.Renderer {
   ...
   */
    code(code, lang, escaped) {
        ...
        const parsed = super.code(code, lang, escaped);
        let renderred = '';
...
function parseMeta(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line[0] === '#') {
            ...
            if (matches) {
                const key = lowerCamelCase(matches[1]);
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].match(PATTERN_START_TOKEN)) {
            lines[i] = lines[i].replace(PATTERN_START_TOKEN, (total, lang) => `\`\`\`\`!${lang}`);
        }
    }
...
module.exports = function (content) {
    ...
    const js = [];
    const css = [];
    const realtiveRoot = path.relative(path.dirname(resourcePath), cwd);
    ...
    // params
    const title = meta.title || resourcePath;
    const layout = meta.layout || 'default';
    const charset = meta.charset || 'utf8';
    const headHtml = renderer.getHeadHtml();
    const tplFile = path.join(__dirname, 'layout', `${layout}.html`);
    if (!fs.existsSync(tplFile)) {
        throw new Error(`读取 layout 文件 ${tplFile} 失败`);
    }
    const tplContent = fs.readFileSync(tplFile, 'utf8');
    const html = template(tplContent, {
        name: pkg.name,
        title,
        charset,
        headHtml,
    ...
    const htmlPath = path.relative(cwd, resourcePath).replace(/\.md$/, '.html');
